...
async function getSessionUser() {
  ...
  await runMigrations();
  return User.findByPk(sessionId, { attributes: ["id", "fullName", "passwordHash"] });
}

export async function GET() {
  const user = await getSessionUser();
  if (!user) return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
  return NextResponse.json({ fullName: String(user.get("fullName")) });
}
...
export async function PUT(request: Request) {
  try {
    ...
    const body = await request.json() as { fullName?: string; currentPassword?: string; newPassword?: string };
    ...
    const updates: { fullName: string; passwordHash?: string } = { fullName };
    if (newPassword) updates.passwordHash = await hashPassword(newPassword);
    await user.update(updates);

    return NextResponse.json({ message: "Admin account updated successfully.", fullName });
  } catch (error) {
