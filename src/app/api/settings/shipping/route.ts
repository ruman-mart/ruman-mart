...
export async function PUT(request: Request) {
  ...
  try {
    ...
    await ShippingSettings.upsert({ id: 1, rates: JSON.stringify(rates), advanceShipping: Boolean(body.advanceShipping), advanceAccountNumber: body.advanceAccountNumber?.trim() || null, advanceAccountTitle: body.advanceAccountTitle?.trim() || null, advanceAccountName: body.advanceAccountName?.trim() || null, logoUrl: body.logoUrl?.trim() || "/logo-web.webp", storeAddress: body.storeAddress?.trim() || null, storePhone: body.storePhone?.trim() || null, storeEmail: body.storeEmail?.trim() || null, facebookUrl: body.facebookUrl?.trim() || null, instagramUrl: body.instagramUrl?.trim() || null, tiktokUrl: body.tiktokUrl?.trim() || null, whatsappUrl: normalizedWhatsappUrl || null, aboutStoryTitle: body.aboutStoryTitle?.trim() || null, aboutStoryText: body.aboutStoryText?.trim() || null, aboutStorySecondText: body.aboutStorySecondText?.trim() || null, aboutStoryImage: body.aboutStoryImage?.trim() || null });
